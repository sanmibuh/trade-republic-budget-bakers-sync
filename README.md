...
**During regular sync** — CANCELED events are skipped entirely and never posted to BudgetBakers. They appear as *Excluded* in sync summaries.

**During forced resync** (`/resync` or `python -m app resync`) — if the transaction was already synced (a BudgetBakers record exists), the resync posts an additional **reversal record** with the opposite sign and a `[Cancelada]` prefix in the note. This keeps a full audit trail without deleting the original entry. After the reversal is posted, the stored ID is cleared so that running the same resync again does not post a second reversal.

> If you see a `[Cancelada]` entry in BudgetBakers, it is an intentional reversal generated by the resync — not a duplicate.
